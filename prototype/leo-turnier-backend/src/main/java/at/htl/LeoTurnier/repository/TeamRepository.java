...
public class TeamRepository implements PanacheRepository<Team> {
    ...
    public Competitor add(Team team) {
        if (team == null || getById(team.getId()) != null) {
            return null;
        }
    ...
    public Competitor modify(long id, Team team) {
        Team toModify = findById(id);
        if (team == null || toModify == null) {
            return null;
        }
        toModify.getPlayers().forEach(p -> {
            playerRepository.getById(p.getId()).setTeam(null);
        });
